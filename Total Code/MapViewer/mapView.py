...
#path = "/Users/Daniel/Documents/PythonStuff/"
...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def sliderval(self):
        ...
        if self.XView.isChecked():
            fd = open(self.name, 'rb')
            self.readslice(fd, self.cubeorder)
    ...
    def file_open(self, args):
        ...
        while True:
            try:
                ...
                self.dt, self.cubeorder, item = self.getPrec(args)

                # get dimensions of data cube. can be guessed
                bool4d = False
                if self.cubeorder == 4:
                    bool4d = True
                self.ndim = self.getSize(args, item, bool4d)
    ...
    def init_plot(self, ):
        ...
        if self.XView.isChecked():
            view = self.XView
            self.Scroll_Vert.setMaximum(self.rows)
            self.Scroll_Horz.setMaximum(40)
        elif self.YView.isChecked():
            view = self.YView
            self.Scroll_Vert.setMaximum(self.cols)
            self.Scroll_Horz.setMaximum(19) #HERE
        elif self.ZView.isChecked():
            view = self.ZView
            self.Scroll_Vert.setMaximum(self.slices)
            self.Scroll_Horz.setMaximum(40)
    ...
    def readslice(self, fd, dim):
        data = np.fromfile(file=fd, dtype=self.dt, sep="")
        data = data.reshape(self.ndim, order='F')
        fd.close()
        if dim == 4:
            data = data[:, :, :, self.Scroll_Horz.value()]
        self.X = data[:, :, :]
        del data
